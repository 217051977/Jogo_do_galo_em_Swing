...
/**
 * Esta classe é fornecida para facilitar o desenho de pixeis no écran.
...
public class Screen extends JPanel {
    ...
    public void reset (int WIDTH, int HEIGHT) {
        ...
        int horizontalSpace = WIDTH/3;
        int verticalSpace = HEIGHT/3;

        for (int y = verticalSpace; y <= verticalSpace*2; y += verticalSpace) {
        ...
        for (int y = 0; y < canvas.getHeight(); y++) {
            for (int x = horizontalSpace; x <= horizontalSpace*2; x += horizontalSpace) {
                pixels[y][x - 1] = color;
                pixels[y][x] = color;
                pixels[y][x + 1] = color;
                repaint();
